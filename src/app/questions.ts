...
export async function generatePages(content: string): Promise<Page[]> {
    const questions = await generateQuestionsFromText(content, 3); // Generate 5 questions
    const pages: Page[] = [];

    for (const question of questions) {
        const page = await generatePage(content, question);
        pages.push(page);
    }
    return pages;
}

export async function generatePage(content: string, question: string): Promise<Page> {
    try {
